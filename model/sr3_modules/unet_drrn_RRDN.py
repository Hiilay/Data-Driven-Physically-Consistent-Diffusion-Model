...
class DRRN(nn.Module):
    def __init__(self):
        ...
        self.donw2 = nn.Conv3d(128, 256, kernel_size=3, stride=1,padding=1)
        self.conv3 = nn.Conv3d(256, 256, kernel_size=3, stride=1,padding=1)
        self.donw3 = nn.Conv3d(256, 512, kernel_size=3, stride=1,padding=1)
...
class Block(nn.Module):
    def __init__(self, dim, dim_out, groups=32, dropout=0):
        ...
        self.block = nn.Sequential(
            ...
            nn.Conv3d(dim, dim_out, 3,1,1),
            nn.ReLU()
        )
    def forward(self, x):
        return self.block(x)

class Block_RRDN(nn.Module):
    def __init__(self, dim, dim_out, groups=32, dropout=0):
        super().__init__()
        # self.block = nn.Sequential(
        #     nn.GroupNorm(groups, dim),
        ...
        # )
        dim2 = int(dim/2)
        self.group = nn.GroupNorm(groups, dim)
        self.conv3d1 = nn.Conv3d(dim, dim2, 3, 1, 1)
        self.conv3d2 = nn.Conv3d(dim+dim2, dim_out, 3, 1, 1)
        # self.conv3d3 = nn.Conv3d(dim+2*dim2, dim_out, 3, 1, 1)   
        self.res = nn.Conv3d(dim, dim_out, 1)
        self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)

    def forward(self, x):
        x = self.group(x)
        x1 = self.lrelu(self.conv3d1(x))
        x2 = self.lrelu(self.conv3d2(torch.cat((x, x1), 1)))
        # #print("X3:   "+str(x3.shape))
        # #print("x:    "+str(x.shape))
        return x2+self.res(x)
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, noise_level_emb_dim=None, dropout=0, use_affine_level=False, norm_groups=32):
        ...
        self.noise_func2 = FeatureWiseAffine(
            noise_level_emb_dim, dim_out, use_affine_level)

        self.block1 = Block_RRDN(dim, dim_out, groups=norm_groups)
        self.block2 = Block(dim_out, dim_out, groups=norm_groups)
        # self.block3 = Block(dim_out, dim_out, groups=norm_groups)
    ...
    def forward(self, x, time_emb):
        b, c, h, w,d = x.shape
        ha = self.block1(x)
        ha = self.noise_func2(ha, time_emb)
        ha = self.block2(ha)
...
class ResnetBlocWithAttn(nn.Module):
    def __init__(self, dim, dim_out, *, noise_level_emb_dim=None, norm_groups=64, dropout=0, with_attn=False):
        super().__init__()
...
class UNet(nn.Module):
    def __init__(
        ...
        self.final_conv = Block_RRDN(pre_channel, default(out_channel, in_channel), groups=norm_groups)
...
if __name__ == '__main__':
    ...
    model = UNet(in_channel=2, out_channel=1, inner_channel=64,norm_groups=16,channel_mults=[1,2,4,8,8],
                 attn_res=[64],res_blocks=1,dropout=0.2,image_size=256).to(device)
    
    x1 = torch.randn(batch_size, 1, 64, 64, 128).to(device)
    x2 = torch.randn(batch_size, 1, 64, 64, 128).to(device)
    x3 = torch.randn(batch_size, 1, 8, 8, 16).to(device)
